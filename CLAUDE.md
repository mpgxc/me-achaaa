...
- **Packaging:** `package.yml` (serverless `package.patterns`) packages the compiled output via `build/**` (the tsconfig `outDir`) — that's where the `serverless.yml` handlers point (`build/handlers/...`). `app/serverless-packaging.test.ts` guards this: it asserts every `handler:` in `serverless.yml` resolves to an existing `app/` source module and that `package.yml` still includes `build/**` (and never a stale `dist/app/handlers/**`), so a rename or a bad pattern can't silently ship an empty/missing handler.
